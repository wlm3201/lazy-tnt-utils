...
import net.orbitalstrike.server.DistanceEnforcer;
import net.orbitalstrike.server.ServerItemTracker;
import net.orbitalstrike.server.ServerTntTracker;
import net.orbitalstrike.server.TickSprintGuard;
    ...
    ServerPlayNetworking.registerGlobalReceiver(
        ...
        (payload, context) -> {
          context
              ...
              .execute(
                  () -> {
                    if (payload.kind().equals("tnt")) SyncConfig.tntServerSync = payload.value();
                    else if (payload.kind().equals("item"))
                      SyncConfig.itemServerSync = payload.value();
                    else return;
                    SyncConfig.save();
